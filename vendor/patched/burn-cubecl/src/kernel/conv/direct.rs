...
fn direct_conv2d_kernel<E: Numeric>(
    ...
    let mut sum = match bias {
        CubeOption::Some(bias) => Line::<f32>::cast_from(bias[out_c as usize / line_size_out]),
        CubeOption::None => Line::<f32>::empty(line_size_out).fill(0f32),
    };
    ...
    output[ABSOLUTE_POS] = Line::<E>::cast_from(sum);
}
